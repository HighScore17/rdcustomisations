...
class Wpcl_Data_Formatters
{
    ...
    )
    {
        ...
        foreach ( $subscriptions_ids as $subscription_id => $subscription_obj ) {
            ...
            if ( $nextpayment !== 0 && $nextpayment !== '-' ) {
                ...
                $next_payments[] = $dt->format( get_option( 'date_format' ) );
            }
     ...
     * @return array
     */
    public static function data_subscription_billing_period(
        $column,
        $source,
        $params,
        $columns,
    ...
    )
    {
        ...
        foreach ( $subscriptions_ids as $subscription_id => $subscription_obj ) {
            $billing_periods[] = $subscription_obj->get_billing_period();
        }
        $value = implode( '<br />', $billing_periods );
        $current_row[$column] = $value;
        return [
            'current_row' => $current_row,
            'columns'     => $columns,
