...
3. A guest user's cart will be captured on the **Checkout page**, if it is abandoned after entering the **email address**.
...
7. Create **unlimited email templates** to be sent at intervals that you set - Intervals start from 1 minute after cart abandonment.
...
11. Use **WooCommerce webhooks** to send abandoned and recovered cart information to a delivery URL of your choice.
...
2. **Exit Intent** Popup Modal - Stop your visitors from leaving the website. Offer special discounts and **redirect the users to the Checkout page** for faster conversions. Capture their email address as well for further communications.
3. **Connect to CRMs** - Integrate your store with email marketing tools or a CRM of your choice. Send user and cart information to these tools by simply setting up the integration once. The plugin will send all the relevant information without any intervention.
4. Works off-the-shelf as it comes with **3 default email templates**.
5. Offer **incentives to customers** to return and complete their checkout with **discounts and coupons**.
6. Add custom variables like **Customer Name, Product Information, Coupons**, etc. in the email template.
7. Embed WooCommerce coupons & also **generate unique coupons** in the emails being sent to customers.
8. Track whether **expired coupons** are causing cart abandonment.
9. Track **emails sent, emails opened, links clicked** for each template/email.
10. **Product report** allows you to see which products are being abandoned & which are being recovered the most.
11. Start sending email templates **within minutes** of cart being abandoned.
12. Admin can send the customer emails to specific abandoned cart(s) using **'Send Custom Email'** feature.
13. If the store is using **WPML** then admin can translate all the abandoned cart reminder email templates using WPML. Then the emails will be sent to the customers in the **same language in which they have abandoned the cart**.
14. Admin can **Print** or export the Abandoned Orders to **CSV** format.
15. Set up different reminder emails based on **user type, cart content, cart total, cart status** and more.
16. Admin can **restrict** the abandoned carts based on the **'Email address'**, **'IP address'** & **'Domain name'**.
17. Admin can take a **glimpse** of **abandoned carts**, **recovered orders**, and the **states of the email template** from the **Dashboard** tab.
18. The plugin will consider the **taxation** which is set in the WooCommerce. The abandoned cart total will be displayed as per the selected tax setting.
...
Abandoned Cart Plugin relies on the Action Scheduler library shipped with WooCommerce to send reminders. This avoids the hassle of creating any new setup and is extremely reliable.
