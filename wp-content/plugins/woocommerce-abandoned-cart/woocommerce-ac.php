...
if ( ! class_exists( 'woocommerce_abandon_cart_lite' ) ) {
	...
	class woocommerce_abandon_cart_lite {
		...
		public function wcal_initialize_plugin_options() {
			...
			add_settings_field(
				'wcal_enable_postmark',
				__( 'Enable Postmark Support', 'woocommerce-abandoned-cart' ),
				array( $this, 'wcal_enable_postmark_callback' ),
				'woocommerce_ac_page',
				'ac_lite_general_settings_section',
				array( __( 'Yes, enable Postmark support.', 'woocommerce-abandoned-cart' ) )
			);
		...
		public static function wcal_enable_postmark_callback( $args ) {

			$enable_postmark = get_option( 'wcal_enable_postmark', '' );

			if ( isset( $enable_postmark ) && '' === $enable_postmark ) {
				$enable_postmark = 'off';
			}
			printf(
				'<input type="checkbox" id="wcal_enable_postmark" name="wcal_enable_postmark" value="on" ' . checked( 'on', $enable_postmark, false ) . ' />'
			);
			$html = '<label for="wcal_enable_postmark"> ' . $args[0] . '</label>';
			echo wp_kses_post( $html );
		}
			...
			if ( is_user_logged_in() ) {
				...
				if ( $gdpr_consent && ! $wcal_user_restricted ) {
					...
					if ( 0 === count( $results ) ) {
						...
						$cart_info_meta['brand'] = function_exists('wc_horizon_get_brand_name') ? wc_horizon_get_brand_name() : 'unknown';
						...
						$updated_cart_info['brand'] = function_exists('wc_horizon_get_brand_name') ? wc_horizon_get_brand_name() : 'unknown';
				...
				if ( $gdpr_consent ) {
					...
					$cart['brand'] = function_exists('wc_horizon_get_brand_name') ? wc_horizon_get_brand_name() : 'unknown';
		...
		public function wcal_preview_email_sent() {
			if ( isset( $_POST['body_email_preview'] ) && '' !== $_POST['body_email_preview'] ) { // phpcs:ignore WordPress.Security.NonceVerification
				...
				if ( isset( $is_wc_template ) && 'true' === $is_wc_template ) {
					...
					$args = woocommerce_abandon_cart_lite::wcal_filter_email_data( $to_email_preview, $subject_email_preview, stripslashes( $final_email_body ), $headers );
					if( !$args["sended"] ) {
						wc_mail( $args["email"], $args["subject"], $args["body"], $args["headers"] );
					}
				} else {
					$args = woocommerce_abandon_cart_lite::wcal_filter_email_data( $to_email_preview, $subject_email_preview, stripslashes( $body_email_preview ), $headers );
					if( !$args["sended"] ) {
						wp_mail( $args["email"], $args["subject"], $args["body"], $args["headers"] );
					}
				}
				do_action( 'wcal_send_email', $to_email_preview, 'TESTING_EMAIL' );
				echo 'email sent';
				die();
			} else {
				echo 'not sent';
		...
		public static function wcal_filter_email_data( $email, $subject, $body, $headers, $variables = array() ) {
			return apply_filters( "wacl_get_email_args", array( 
				"email" => $email, 
				"subject" => $subject, 
				"body" => $body,
				"headers" => $headers,
